...
3. **模糊年資數據**：由於研發到銷售實際僅 7、8 年，網頁上已移除了具體年份數字（如 "10年" 或 "30年"），以保持嚴謹。
   * *合格數據指標展示*：
     * 數值：`專業` ➡️ 標籤：`在地核心技術研發`。
...
* ✂️ **邊緣裁剪**：向內裁剪所有四個邊緣的 **5%**，去除 AI 繪圖模型在角落或邊界產生的瑕疵線條或重疊殘影。
...
4. **色票分類按鈕間距**：色彩分類按鈕的容器 `.palette-nav` 必須設定 `row-gap: 1.2rem;`，並將底線定位控制在 `bottom: -0.7rem;`，防止第一排按鈕選取時的底線重疊到第二排的文字。
